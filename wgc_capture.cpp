...
WGCCapture::WGCCapture(bool use_bgra) : is_capturing_(false), last_texture_timestamp_(0), has_new_texture_(false) {
...
bool WGCCapture::start_capture() {
    ...
    try {
        ...
        frame_pool_.FrameArrived([this](auto&& sender, auto&&) {
            ...
            // Сохраняем текстуру и отмечаем, что у нас есть новая текстура
            {
                std::lock_guard<std::mutex> lock(frame_mutex_);
                // Обновляем последнюю текстуру
